...
from scheduler_testbed.estimation.ukf_utils import findNearestPositiveDefiniteMatrix
...
class UnscentedKalmanFilter(Filter):
    ...
    def update(self, measurement: ndarray | None):
        ...
        if type(measurement) == type(None):
            ...
            if measurement.ndim > 1:
                raise ValueError("Measurement must be a (M,) array, but has ndim > 1")
    ...
    def predictCovariance(self, final_time: float):
        ...
        self.pred_p = (
            self.sigma_x_res.dot(self.cvr_weight.dot(self.sigma_x_res.T)) + self.q_matrix
        )
    ...
    def calculateInnovations(self, measurement: ndarray):
        ...
        Args:
            measurement (``ndarray``): (M,) array of measurements
        """
        self.innovation = self.calcMeasurementResiduals(measurement, self.mean_pred_y)
        self.nis = self.innovation.T.dot(inv(self.innov_cvr).dot(self.innovation))
